...
import (
	...
	"github.com/dariocasas/deputados3/src/pkg/runner"
...
func (p PopulateDbUseCase) Execute(input PopulateDbInputDTO) *PopulateDbOutputDTO {
	...
	populateOne := func(idx *deputado_index.DeputadoId) *PopulateDbOutputDTO {
	...
	f := func(in runner.InputChanType) runner.OuputChanType {
		return runner.OuputChanType{
			Value: populateOne(&deputado_index.DeputadoId{
				Id: in.Value.(*deputado_index.DeputadoId).Id},
			),
		}
	}
	...
	for n := range sinkChan {
		log.Printf("Duration ms %d ", n.Value.(*PopulateDbOutputDTO).Duration.Milliseconds())
		itemsProcessed++
	}
	log.Printf("PopulateDbUseCase.Execute itemsProcessed:%d", itemsProcessed)
...
func generateSourceChannel(done chan struct{}, concurrency int, nums deputado_index.DeputadoIndex) <-chan runner.InputChanType {
	...
	out := make(chan runner.InputChanType, concurrency)
